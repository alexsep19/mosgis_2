...
import ru.eludia.products.mosgis.rest.api.OrganizationMemberFilesLocal;
...
public class OrganizationMemberFilesImpl extends BaseCRUD<OrganizationMemberFile> implements OrganizationMemberFilesLocal  {

    private static final Logger logger = Logger.getLogger (OrganizationMemberFilesImpl.class.getName ());
    ...
    public JsonObject doCreate (JsonObject p, User user) {return fetchData ((db, job) -> {
        ...
        db.begin ();
            ...
            Object id = db.insertId (getTable (), HASH (
                ...
                "uuid_org_member",  file.getString ("uuid_org_member"),
    ...
    public JsonObject doUpdate (String id, JsonObject p, User user) {return doAction (db -> {
        ...
        final OrganizationMemberFile table = (OrganizationMemberFile) getTable ();
        ...
        try (PreparedStatement st = cn.prepareStatement ("SELECT body, DBMS_LOB.GETLENGTH(body) FROM " + table.getName () + " WHERE uuid = ? FOR UPDATE")) {
            ...
            String parent = db.getString(new QP("SELECT uuid_org_member FROM tb_org_member_files WHERE uuid = ?", id));

            db.update(getTable(), HASH(
                    "uuid_org_member", parent,
                    "is_deleted", 1
            ), "uuid_org_member");
...
//        publishMessage (action, id_log);
    ...
    public JsonObject doDelete (String id, User user) {return doAction (db -> {
        ...
        JsonObject houseFile = db.getJsonObject (ModelHolder.getModel ().get (OrganizationMemberFile.class, id, "*"));

        db.update (OrganizationMemberFile.class, HASH (
            "uuid",      id,
            "id_status", 2
        ));
    ...
    public JsonObject select (JsonObject p, User u) {return fetchData ((db, job) -> {
        ...
        if (!s.getFilters ().containsKey ("uuid_org_member"))
            throw new IllegalStateException ("uuid_org_member filter is not set");

        Select select = ModelHolder.getModel ()
            .select (getTable (), "*", "uuid AS id")
            .toMaybeOne (OrganizationMemberDocument.class, "AS obj").on ()
            .where  ("id_status",  1);
