...
@MessageDriven(activationConfig = {
    @ActivationConfigProperty(propertyName = "destinationLookup", propertyValue = "mosgis.inImportCitizenCompensationCategoriesQueue")
    , @ActivationConfigProperty(propertyName = "subscriptionDurability", propertyValue = "Durable")
    , @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue")
})
public class ImportCitizenCompensationCategoriesMDB extends UUIDMDB<InCitizenCompensationCategory> {
    
    private static final Logger logger = Logger.getLogger (ImportCitizenCompensationCategoriesMDB.class.getName ());
    ...
    protected void handleRecord (DB db, UUID uuid, Map r) throws SQLException {
		...
		"uuid_ack", wsGisNsiClient.exportDataProviderNsiItem((UUID) r.get("ppa"), uuid, 302).getMessageGUID()
            ...
            logger.log (Level.SEVERE, "Cannot import citizen compensation categories", ex);
        }
