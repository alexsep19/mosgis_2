    ...
    public void importLicenses() {
        ...
        try (DB db = model.getDb()) {
            db.forEach(model
                    ...
                    .toOne(VocOrganizationNsi20.class, "AS role",  "uuid").on("root.uuid = role.uuid AND is_deleted = 0 AND role.code = '1'")
                    .where("ogrn IS NOT NULL")
                    , rs -> {
                UUIDPublisher.publish(queue, (UUID) db.insertId(InLicenses.class, DB.HASH("uuid_org", rs.getObject("uuid"))));
            });
        } catch (Exception ex) {
