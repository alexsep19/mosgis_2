        ...
        trigger ("BEFORE UPDATE", ""
                ...
                + "BEGIN "
                    ...
                    + "IF :NEW.code_vc_nsi_33 <> :OLD.code_vc_nsi_33 THEN BEGIN "
                        ...
                        + "IF cnt > 0 THEN  BEGIN "
                            + "SELECT COUNT(*) INTO cnt FROM tb_oki_resources res WHERE res.uuid_oki = :NEW.uuid AND res.is_deleted = 0; "
                            + "IF cnt > 0 THEN "
                                + "raise_application_error (-20000, 'К данному объекту привязаны характеристики мощностей объекта. Операция отменена'); "
                            + "END IF; END; "
                        + "ELSE BEGIN "
                            + "SELECT COUNT(*) INTO cnt FROM tb_oki_tr_resources res WHERE res.uuid_oki = :NEW.uuid AND res.is_deleted = 0; "
                            + "IF cnt > 0 THEN "
                            ...
                            + "END IF; "
                            + "SELECT COUNT(*) INTO cnt FROM tb_oki_net_pieces net WHERE net.uuid_oki = :NEW.uuid AND net.is_deleted = 0; "
                            + "IF cnt > 0 THEN "
