...
@MessageDriven(activationConfig = {
 @ActivationConfigProperty(propertyName = "destinationLookup", propertyValue = "mosgis.outExportHouseSupplyResourceContractsQueue")
 , @ActivationConfigProperty(propertyName = "subscriptionDurability", propertyValue = "Durable")
 , @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue")
})
public class GisPollExportSupplyResourceContractMDB  extends GisPollMDB {
    ...
    protected Get get (UUID uuid) {
        return (Get) ModelHolder.getModel ().get (getTable (), uuid, "AS root", "*")                
            .toOne (SupplyResourceContractLog.class,     "AS log", "uuid", "action").on ("log.uuid_out_soap=root.uuid")
            .toOne (SupplyResourceContract.class,        "AS ctr", "uuid", "id_ctr_status").on ()
    ...
    protected void handleOutSoapRecord (DB db, UUID uuid, Map<String, Object> r) throws SQLException {
        ...
        try {
            ...
            if (action != SupplyResourceContract.Action.ANNULMENT) {
                ...
                if (DB.ok (guid)) h.put (c.CONTRACTGUID.lc (), guid);
		final String rootguid = commonResult.get(0).getImportSupplyResourceContract().getContractRootGUID();
		if (DB.ok(rootguid))  h.put(c.CONTRACTROOTGUID.lc(), rootguid);
            }
...
logger.info ("h=" + h);
        ...
        db.update (SupplyResourceContract.class, h);
        
        h.put ("uuid", uuid);
        db.update (SupplyResourceContractLog.class, h);
