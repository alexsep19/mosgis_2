...
//        r.put (c.NOTLINKEDWITHMETERING.lc (), 1 - toBool (row, 13, "Не указано, определяется ли объём ресурсов несколькими ПУ"));
    ...
    public InXlMeteringDevice () {
        ...
        trigger ("BEFORE INSERT", ""
            ...
            + "      union "
            + "      select b.uuid FROM tb_blocks b, items i where i.uuid_premise = b.uuid and b.blocknum = :NEW.PREMISESNUM "
            + "      union "
            + "      select l.uuid FROM tb_living_rooms l, items i where i.uuid_premise = l.uuid and l.roomnumber = :NEW.ROOMNUMBER and "
            + "            (exists(select 1 from tb_premises_res lr where lr.uuid = l.uuid_premise and lr.premisesnum = :NEW.PREMISESNUM) or "
            + "             exists(select 1 from tb_blocks lb where lb.uuid = l.uuid_block and lb.blocknum = :NEW.PREMISESNUM)) "
            + "      ); "
