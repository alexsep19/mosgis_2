...
import ru.eludia.products.mosgis.rest.api.SupplyResourceContractDocsLocal;
import ru.eludia.products.mosgis.ws.rest.impl.base.BaseCRUD;
import ru.eludia.products.mosgis.ws.rest.impl.tools.ComplexSearch;

@Stateless
@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
public class SupplyResourceContractDocsImpl extends BaseCRUD<SupplyResourceContractFile> implements SupplyResourceContractDocsLocal  {

    private static final Logger logger = Logger.getLogger (SupplyResourceContractDocsImpl.class.getName ());

    private static final String PARENT_REF = SupplyResourceContractFile.c.UUID_SR_CTR.lc ();
    ...
    public JsonObject doUpdate (String id, JsonObject p, User user) {return doAction (db -> {
        ...
        final SupplyResourceContractFile table = (SupplyResourceContractFile) getTable ();
    ...
    public JsonObject doDelete (String id, User user) {return doAction (db -> {
        ...
        JsonObject houseFile = db.getJsonObject (ModelHolder.getModel ().get (SupplyResourceContractFile.class, id, "*"));
    ...
    public JsonObject select (JsonObject p, User u) {return fetchData ((db, job) -> {
        ...
        Select select = ModelHolder.getModel ()
            ...
            .toMaybeOne (SupplyResourceContract.class, "AS obj").on ()
            .where  ("id_status",  1);
