...
import ru.eludia.products.mosgis.db.model.tables.OverhaulAddressProgram.c;
...
@MessageDriven(activationConfig = {
 @ActivationConfigProperty(propertyName = "destinationLookup", propertyValue = "mosgis.outExportOverhaulAddressProgramsQueue")
 , @ActivationConfigProperty(propertyName = "subscriptionDurability", propertyValue = "Durable")
 , @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue")
})
public class GisPollExportOverhaulAddressProgramsMDB extends GisPollMDB {
    ...
    protected Get get (UUID uuid) {
        ...
        return (Get) ModelHolder.getModel ().get (getTable (), uuid, "AS root", "*")                
            .toOne (OverhaulAddressProgramLog.class,     "AS log", "uuid", "action", "id_oap_status", "uuid_user AS user").on ("log.uuid_out_soap=root.uuid")
            .toOne (OverhaulAddressProgram.class,        "AS program", "uuid").on ()
    ...
    protected void handleOutSoapRecord (DB db, UUID uuid, Map<String, Object> r) throws SQLException {
        ...
        OverhaulAddressProgram.Action action = OverhaulAddressProgram.Action.forStatus (VocGisStatus.i.forId (r.get ("log.id_oap_status")));
        ...
        try {
            ...
            switch (action) {            
                case PROJECT_PUBLISH:
                    ...
                    nextQueue = inExportOverhaulAddressProgramHouseWorksManyQueue;
                    break;
                case PROJECT_DELETE:
                case ANNUL:
                    List <Map <String, Object>> works = db.getList (db.getModel ()
                        .select (OverhaulAddressProgramHouseWork.class, "AS works", "*")
                            .toOne (OverhaulAddressProgramHouse.class, "AS houses").on ()
                                .toOne (OverhaulAddressProgram.class, "AS program").where ("uuid", r.get ("program.uuid")).on ("program.uuid = houses.program_uuid")
                        .where (OverhaulAddressProgramHouseWork.c.ID_OAPHW_STATUS.lc (), VocGisStatus.i.APPROVED.getId ())
                    );
                    
                    works.forEach ((map) -> {
                        map.put (OverhaulAddressProgramHouseWork.c.ID_OAPHW_STATUS.lc (), VocGisStatus.i.ANNUL.getId ());
                    });
            ...
            if (nextQueue != null) {
                String importWorksId = db.insertId (OverhaulAddressProgramHouseWorksImport.class, HASH (
                    OverhaulAddressProgramHouseWorksImport.c.PROGRAM_UUID.lc (), r.get ("program.uuid"),
                    OverhaulAddressProgramHouseWorksImport.c.ORGPPAGUID.lc (),   orgPPAGuid
                )).toString ();
                List <Map <String, Object>> works = db.getList (db.getModel ()
                    .select (OverhaulAddressProgramHouseWork.class, "AS works", "*")
                        .toOne (OverhaulAddressProgramHouse.class, "AS houses").where ("is_deleted", 0).on ()
                            .toOne (OverhaulAddressProgram.class, "AS programs").where ("uuid", r.get ("program.uuid")).on ("programs.uuid=houses.program_uuid")
                    .where ("is_deleted", 0)
...
logger.info ("h=" + h);
        ...
        db.update (OverhaulAddressProgram.class, h);
        
        h.put ("uuid", uuid);
        db.update (OverhaulAddressProgramLog.class, h);
