...
import ru.eludia.products.mosgis.rest.api.OverhaulShortProgramHousesLocal;

@Stateless
@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
public class OverhaulShortProgramHousesImpl extends BaseCRUD <OverhaulShortProgramHouse> implements OverhaulShortProgramHousesLocal {
    
    @Override
    public JsonObject select (JsonObject p, User user) {return fetchData ((db, job) -> {
        
        Select select = ModelHolder.getModel ().select (OverhaulShortProgramHouse.class, "AS root", "*")
    ...
    public JsonObject getItem (String id, User user) {return fetchData ((db, job) -> {
        ...
        job.add ("item", db.getJsonObject (ModelHolder.getModel ()
            .get        (getTable (), id, "*")
            .toOne      (OverhaulShortProgram.class, "AS program", "startyear AS startyear", 
                                                                   "endyear AS endyear", 
    ...
    public JsonObject getLog (String id, JsonObject p, User user) {return fetchData ((db, job) -> {
        ...
        Select select = ModelHolder.getModel ().select (OverhaulShortProgramHouseLog.class, "AS log", "*", "uuid AS id")
            .and ("uuid_object", id)
            .toOne      (OverhaulShortProgramHouse.class, "AS object").on ()
