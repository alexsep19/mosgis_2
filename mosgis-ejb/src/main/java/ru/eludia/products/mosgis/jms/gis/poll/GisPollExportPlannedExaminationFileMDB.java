...
@MessageDriven(activationConfig = {
    @ActivationConfigProperty(propertyName = "destinationLookup", propertyValue = "mosgis.outExportPlannedExaminationFilesQueue")
    , @ActivationConfigProperty(propertyName = "subscriptionDurability", propertyValue = "Durable")
    , @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue")
})
public class GisPollExportPlannedExaminationFileMDB extends UUIDMDB<PlannedExaminationFile> {
	...
	protected Get get(UUID uuid) {
		return (Get) ModelHolder.getModel().get(getTable(), uuid, "attachmentguid")
				.toOne(PlannedExamination.class, "AS ex").on()
				.toOne(VocOrganization.class, "AS org", "orgppaguid").on("ex.regulator_uuid");
	}
    
    @Override
    protected void handleRecord (DB db, UUID uuid, Map<String, Object> r) throws SQLException {       
        
        Response rp = restGisFilesClient.get ((UUID) r.get ("org.orgppaguid"), RestGisFilesClient.Context.INSPECTION, (UUID) r.get ("attachmentguid"));
