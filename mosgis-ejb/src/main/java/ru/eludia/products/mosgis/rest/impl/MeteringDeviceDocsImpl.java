...
import ru.eludia.products.mosgis.rest.api.MeteringDeviceDocsLocal;
import ru.eludia.products.mosgis.db.model.tables.MeteringDeviceFile;
...
public class MeteringDeviceDocsImpl extends BaseCRUD<MeteringDeviceFile> implements MeteringDeviceDocsLocal  {

    private static final Logger logger = Logger.getLogger (MeteringDeviceDocsImpl.class.getName ());    
    ...
    public JsonObject doUpdate (String id, JsonObject p, User user) {return doAction (db -> {
        ...
        final MeteringDeviceFile table = (MeteringDeviceFile) getTable ();
...
//        publishMessage (action, id_log);
    ...
    public JsonObject doDelete (String id, User user) {return doAction (db -> {
        ...
        JsonObject houseFile = db.getJsonObject (ModelHolder.getModel ().get (MeteringDeviceFile.class, id, "*"));
            
        db.update (MeteringDeviceFile.class, HASH (
            "uuid",      id,
            "id_status", 2
        ));
    ...
    public JsonObject select (JsonObject p, User u) {return fetchData ((db, job) -> {
        ...
        String key = MeteringDeviceFile.c.UUID_METER.lc ();
