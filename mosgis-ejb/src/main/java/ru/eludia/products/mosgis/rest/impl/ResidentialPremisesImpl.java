...
public class ResidentialPremisesImpl extends BasePassport<ResidentialPremise> implements ResidentialPremisesLocal {
    ...
    public JsonObject getItem (String id) {return fetchData ((db, job) -> {
        ...
        db.addJsonArrays (job, 
            ...
            ModelHolder.getModel ().select (
                ...
                .where ("uuid_house", item.getString ("uuid_house"))
                .and   ("is_deleted",  0)
                .orderBy ("entrancenum")
        ...
        JsonObject file = db.getJsonObject (ModelHolder.getModel ().select (HouseFile.class, "*").where ("uuid_premise_res", id).and ("id_status", 1));
    ...
    public JsonObject select (JsonObject p, User user) {return fetchData ((db, job) -> {
        ...
        Select select = ModelHolder.getModel ()                
                ...
                .where ("is_deleted", 0)
                .toMaybeOne (Entrance.class, "entrancenum").on ()
                .orderBy ("entrancenum")
                .orderBy ("premisesnum");
