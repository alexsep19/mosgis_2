...
@MessageDriven(activationConfig = {
 @ActivationConfigProperty(propertyName = "destinationLookup", propertyValue = "mosgis.inExportAcknowledgmentsQueue")
 , @ActivationConfigProperty(propertyName = "subscriptionDurability", propertyValue = "Durable")
 , @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue")
})
public class ExportAcknowledgmentMDB extends GisExportMDB<AcknowledgmentLog> {
    
    @EJB
    WsGisBillsClient wsGisBillsClient;
    
    @Resource (mappedName = "mosgis.outExportAcknowledgmentsQueue")
    Queue outExportAcknowledgmentsQueue;
                    
    protected Get get (UUID uuid) {        
        final MosGisModel m = ModelHolder.getModel ();
        return m.get (AcknowledgmentLog.class, uuid, "*");
    }
        
    AckRequest.Ack invoke (DB db, Acknowledgment.Action action, UUID messageGUID,  Map<String, Object> r) throws Fault, SQLException {
            
        UUID orgPPAGuid = (UUID) r.get ("orgppaguid");
            
        switch (action) {
            case PLACING: return wsGisBillsClient.importAcknowledgment (orgPPAGuid, messageGUID, r);
    ...
    protected void handleRecord (DB db, UUID uuid, Map<String, Object> r) throws SQLException {
        ...
        r = AcknowledgmentLog.getForExport (db, uuid.toString ());
        
        VocGisStatus.i status = VocGisStatus.i.forId (r.get ("r." + Acknowledgment.c.ID_CTR_STATUS.lc ()));
        
        Acknowledgment.Action action = Acknowledgment.Action.forStatus (status);        
    ...
    Queue getQueue (Acknowledgment.Action action) {        
        return outExportAcknowledgmentsQueue;        
    }
