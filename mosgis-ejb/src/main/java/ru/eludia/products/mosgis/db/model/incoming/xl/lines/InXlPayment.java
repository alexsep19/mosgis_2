...
public class InXlPayment extends EnTable {
	...
	try {
	    ...
	    throw new XLException ("Укажите период оплаты в формате ММ.ГГГГ, например: 05.2019");
	}

	r.put(c.PAYMENTDOCUMENTID.lc(), toString(row, 4));
	r.put(c.SERVICEID.lc(), toString(row, 5));
    }
	...
	key ("uuid_xl", c.UUID_XL);
	    ...
	    + " IF :NEW.serviceid IS NOT NULL AND :NEW.paymentdocumentid IS NOT NULL THEN "
	    ...
	    + "   SELECT uuid INTO :NEW.uuid_pay_doc FROM " + PaymentDocument.TABLE_NAME + " WHERE is_deleted=0 AND paymentdocumentid=:NEW.paymentdocumentid; "
	    + "   :NEW.id_type := " + VocPaymentBaseType.i.PAYMENT_DOCUMENT + "; "
	    + " END IF; "

	    + " IF :NEW.serviceid IS NOT NULL THEN "
            + "   SELECT COUNT(*), MIN(serviceid) INTO cnt, :NEW.serviceid FROM " + Account.TABLE_NAME + " WHERE is_deleted=0 AND serviceid=:NEW.serviceid; "
	    ...
	    + "   SELECT uuid INTO :NEW.uuid_account FROM " + Account.TABLE_NAME + " WHERE is_deleted=0 AND serviceid=:NEW.serviceid; "
	    + "   :NEW.id_type := " + VocPaymentBaseType.i.ACCOUNT + "; "
	    + " END IF; "
	...
	for (Payment.c c: Payment.c.values ()) if (c.isToXlImport()) {
            sb.append (',');
            sb.append (c.lc ());
            nsb.append (",:NEW.");
            nsb.append (c.lc ());
