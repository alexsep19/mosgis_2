...
public class InXlSupplyResourceContractQualityLevel extends EnTable {
	...
	CODE_VC_NSI_239         (Type.STRING, 20, null, "Ссылка на НСИ \"Вид коммунального ресурса\" (реестровый номер 239)"),
    ...
    private static final Logger logger = Logger.getLogger(InXlSupplyResourceContractQualityLevel.class.getName());
    ...
    public static Map<String, Object> toHash (UUID uuid, int ord, XSSFRow row, Map<String, Map<String, Object>> vocs) {
        ...
        Map<String, Object> r = DB.HASH (
            ...
            SupplyResourceContractQualityLevel.c.UUID_XL, uuid,
            c.ORD, ord    
        );
	...
	r.put(c.CODE_VC_NSI_3.lc(), vocs.get("vc_nsi_3").get(servicetype));
	...
	r.put(c.CODE_VC_NSI_239.lc(), vocs.get("vc_nsi_239").get(municipalresource));


	r.put(SupplyResourceContractQualityLevel.c.CODE_VC_NSI_276.lc(), vocs.get("vc_nsi_276").get(toString(row, 6)));

	if (r.get(SupplyResourceContractQualityLevel.c.CODE_VC_NSI_276.lc()) == null) {
	    throw new XLException("Не удалось определить показатель качества");
	}


	r.put(SupplyResourceContractQualityLevel.c.INDICATORVALUE.lc(), toNumeric(row, 7));

	r.put(SupplyResourceContractQualityLevel.c.INDICATORVALUE_FROM.lc(), toNumeric(row, 8));

	r.put(SupplyResourceContractQualityLevel.c.INDICATORVALUE_TO.lc(), toNumeric(row, 9));

	r.put(SupplyResourceContractQualityLevel.c.INDICATORVALUE_IS.lc()
	    , DB.to.String(toString(row, 10)).toLowerCase().equals("соответствует")? 1 : 0
	);


	r.put(SupplyResourceContractQualityLevel.c.CODE_VC_OKEI.lc(), toNumeric(row, 11));

	r.put(SupplyResourceContractQualityLevel.c.ADDITIONALINFORMATION.lc(), toString(row, 12));
    }

    public InXlSupplyResourceContractQualityLevel() {

	super("in_xl_sr_ctr_qls", "Строки импорта показателей качества поставляемых ресурсов ДРСО");

	cols(c.class);

	for (ColEnum o : SupplyResourceContractQualityLevel.c.values()) {

	    SupplyResourceContractQualityLevel.c c = (SupplyResourceContractQualityLevel.c) o;
	...
	key("uuid_xl", SupplyResourceContractQualityLevel.c.UUID_XL);
	...
	for (SupplyResourceContractQualityLevel.c c : SupplyResourceContractQualityLevel.c.values()) {
	    if (c.isToXlImport()) {
		String col = c.lc();
		sb_fields.append(", ");
	...
	trigger("BEFORE UPDATE", ""
	    ...
	    + " INSERT INTO tb_sr_ctr_qls (uuid,is_deleted,uuid_sr_ctr,uuid_sr_ctr_subj" + sb_fields + ") VALUES (:NEW.uuid,0,:NEW.uuid_sr_ctr,:NEW.uuid_sr_ctr_subj" + sb_insert + "); "
	    + " UPDATE tb_sr_ctr_qls SET is_deleted = 1 WHERE uuid = :NEW.uuid; "
