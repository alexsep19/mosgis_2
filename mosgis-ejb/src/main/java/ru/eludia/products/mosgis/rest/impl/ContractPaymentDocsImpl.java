...
public class ContractPaymentDocsImpl extends BaseCRUD<ContractPaymentFile> implements ContractPaymentDocsLocal  {
    ...
    public JsonObject doUpdate (String id, JsonObject p, User user) {return doAction (db -> {
        ...
        final ContractPaymentFile table = (ContractPaymentFile) getTable ();
...
//        publishMessage (action, id_log);
    ...
    public JsonObject doDelete (String id, User user) {return doAction (db -> {
        ...
        JsonObject houseFile = db.getJsonObject (ModelHolder.getModel ().get (ContractPaymentFile.class, id, "*"));
    ...
    public JsonObject select (JsonObject p, User u) {return fetchData ((db, job) -> {
        ...
        if (!s.getFilters ().containsKey ("uuid_contract_payment")) throw new IllegalStateException ("uuid_contract_payment filter is not set");
