...
@MessageDriven(activationConfig = {
 @ActivationConfigProperty(propertyName = "destinationLookup", propertyValue = "mosgis.inExportPaymentDocumentsQueue")
 , @ActivationConfigProperty(propertyName = "subscriptionDurability", propertyValue = "Durable")
 , @ActivationConfigProperty(propertyName = "destinationType", propertyValue = "javax.jms.Queue")
})
public class ExportPaymentDocumentMDB extends GisExportMDB<PaymentDocumentLog> {
    
    @EJB
    WsGisBillsClient wsGisBillsClient;
    
    @Resource (mappedName = "mosgis.outExportPaymentDocumentsQueue")
    Queue outExportPaymentDocumentsQueue;
                    
    protected Get get (UUID uuid) {        
        final MosGisModel m = ModelHolder.getModel ();
        return m.get (PaymentDocumentLog.class, uuid, "*");
    }
        
    AckRequest.Ack invoke (DB db, PaymentDocument.Action action, UUID messageGUID,  Map<String, Object> r) throws Fault, SQLException {
            
        UUID orgPPAGuid = (UUID) r.get ("orgppaguid");
            
        switch (action) {
            case PLACING: return wsGisBillsClient.importPaymentDocumentData (orgPPAGuid, messageGUID, r);
    ...
    protected void handleRecord (DB db, UUID uuid, Map<String, Object> r) throws SQLException {
        ...
        r = PaymentDocumentLog.getForExport (db, uuid.toString ());
        
        VocGisStatus.i status = VocGisStatus.i.forId (r.get ("r." + PaymentDocument.c.ID_CTR_STATUS.lc ()));
        
        PaymentDocument.Action action = PaymentDocument.Action.forStatus (status);        
    ...
    Queue getQueue (PaymentDocument.Action action) {        
        return outExportPaymentDocumentsQueue;        
    }
    ...
    protected Col getStatusCol () {
        return PaymentDocument.c.ID_CTR_STATUS.getCol ();
    }
