...
import ru.eludia.products.mosgis.rest.api.CharterDocsLocal;
...
public class CharterDocsImpl extends BaseCRUD<CharterFile> implements CharterDocsLocal  {

    private static final Logger logger = Logger.getLogger (CharterDocsImpl.class.getName ());    
    ...
    public JsonObject doCreate (JsonObject p, User user) {return fetchData ((db, job) -> {
        ...
        db.begin ();
            ...
            Object id = db.insertId (getTable (), HASH (
                ...
                "uuid_charter",         file.getString ("uuid"),
                "uuid_charter_object",  file.getString ("uuid_charter_object", null),
    ...
    public JsonObject doUpdate (String id, JsonObject p, User user) {return doAction (db -> {
        ...
        final CharterFile table = (CharterFile) getTable ();
...
//        publishMessage (action, id_log);
    ...
    public JsonObject doDelete (String id, User user) {return doAction (db -> {
        ...
        JsonObject houseFile = db.getJsonObject (ModelHolder.getModel ().get (CharterFile.class, id, "*"));
