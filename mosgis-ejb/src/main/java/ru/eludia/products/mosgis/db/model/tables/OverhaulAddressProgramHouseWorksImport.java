...
public class OverhaulAddressProgramHouseWorksImport extends EnTable {
    
    public enum c implements EnColEnum {
        
        ORGPPAGUID    (UUID,           "Идентификатор зарегистрированной организации"),
        PROGRAM_UUID  (OverhaulAddressProgram.class, "Адресная программа"),
    ...
    public OverhaulAddressProgramHouseWorksImport () {
        
        super   ("tb_oh_addr_pr_works_imports", "Записи импорта видов работ адресной программы капитального ремонта");
    ...
    public static Map <String, Object> getForExport (DB db, String id) throws SQLException {
        ...
        List <Map <String, Object>> works = db.getList (db.getModel ()
            .select (OverhaulAddressProgramHouseWork.class, "AS oh_work", "*")
                .toOne (OverhaulAddressProgramHouseWorksImport.class, "AS imports").where ("uuid", id).on ()
                .toOne  (OverhaulAddressProgramHouse.class, "AS oh_house").on ()
        ...
        Map <String, Object> importData = db.getMap (db.getModel ()
            .get (OverhaulAddressProgramHouseWorksImport.class, id, "orgppaguid")
                .toOne (OverhaulAddressProgram.class, "planguid AS planguid").on ()
        );
