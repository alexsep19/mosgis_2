...
public class SocialNormTarifImpl extends BaseCRUD<SocialNormTarif> implements SocialNormTarifLocal {
    ...
    private void applySimpleSearch (final SimpleSearch search, Select select) {
        ...
        select.and (SocialNormTarif.c.NAME.lc () + " LIKE ?%", searchString.toUpperCase ());
    ...
    private void checkFilter (JsonObject data, SocialNormTarif.c field, Select select) {
        String key = field.lc ();
	...
	final Model m = ModelHolder.getModel();
	    ...
	    .toMaybeOne(VocOkei.class, "AS okei", "*").on()
            .toMaybeOne (SocialNormTarifLog.class).on ()
	...
	db.addJsonArrays(job,
	    m.select(SocialNormTarifOktmo.class, "AS oktmos", "*")
	...
	final JsonObject item = db.getJsonObject (m
            ...
            .toMaybeOne (SocialNormTarifLog.class, "AS log").on ()
	...
	db.addJsonArrays(jb,
	    VocOkei.getVocSelect().where("code IN", Arrays.asList(VocOkei.CODES_ENERGY_WT.split(",")).toArray())
	);
    });}
	...
	if (p.getJsonObject("data").containsKey("oktmo") && !p.getJsonObject("data").isNull("oktmo") ) {
	    ...
	    SocialNormTarifOktmo.store(db, insertId.toString()
	...
	db.update(SocialNormTarif.class, r);

	if (data.containsKey("oktmo") && !data.isNull("oktmo")) {

	    SocialNormTarifOktmo.store(db, id
    ...
    public JsonObject doApprove (String id, User user) {return doAction ((db) -> {
        ...
        db.update (getTable (), HASH (EnTable.c.UUID,               id,
            SocialNormTarif.c.ID_CTR_STATUS, VocGisStatus.i.PENDING_RQ_PLACING.getId ()
        ));
    ...
    public JsonObject doAlter (String id, User user) {return doAction ((db) -> {
        ...
        final Map<String, Object> r = HASH (
            EnTable.c.UUID,               id,
            SocialNormTarif.c.ID_CTR_STATUS,  VocGisStatus.i.PROJECT.getId ()
        );
    ...
    public JsonObject doAnnul (String id, JsonObject p, User user) {return doAction ((db) -> {
        ...
        final Map<String, Object> r = getData(p,
            EnTable.c.UUID,               id,
            SocialNormTarif.c.ID_CTR_STATUS,  VocGisStatus.i.PENDING_RQ_ANNULMENT.getId ()
        );
