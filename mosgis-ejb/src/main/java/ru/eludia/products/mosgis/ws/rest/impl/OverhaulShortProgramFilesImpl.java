...
import ru.eludia.products.mosgis.rest.api.OverhaulShortProgramFilesLocal;
import ru.eludia.products.mosgis.ws.rest.impl.base.BaseCRUD;
import ru.eludia.products.mosgis.ws.rest.impl.tools.ComplexSearch;

@Stateless
@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
public class OverhaulShortProgramFilesImpl extends BaseCRUD <OverhaulShortProgramFile> implements OverhaulShortProgramFilesLocal {
    
    private static final Logger logger = Logger.getLogger (OverhaulShortProgramFilesImpl.class.getName ());
...
//        publishMessage (action, id_log);
    ...
    public JsonObject doUpdate (String id, JsonObject p, User user) {return doAction (db -> {
        ...
        final OverhaulShortProgramFile table = (OverhaulShortProgramFile) getTable ();
    ...
    public JsonObject doDelete (String id, User user) {return doAction (db -> {
        ...
        JsonObject file = db.getJsonObject (ModelHolder.getModel ().get (OverhaulShortProgramFile.class, id, "*"));
            
        db.update (OverhaulShortProgramFile.class, HASH (
            "uuid",      id,
            "id_status", 2
        ));
    ...
    public JsonObject select (JsonObject p, User u) {return fetchData ((db, job) -> {
        ...
        String key = OverhaulShortProgramFile.c.UUID_OH_SHRT_PR_DOC.lc ();
