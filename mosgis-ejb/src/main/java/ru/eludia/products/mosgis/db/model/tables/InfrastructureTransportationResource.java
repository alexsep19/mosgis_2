    ...
    public InfrastructureTransportationResource () {
        ...
        trigger ("BEFORE INSERT OR UPDATE", ""
                ...
                + "BEGIN "
                    ...
                    + "END IF; "
                    + "FOR i IN (SELECT res.uuid FROM tb_oki_tr_resources res WHERE res.uuid_oki = :NEW.uuid_oki AND res.code_vc_nsi_2 = :NEW.code_vc_nsi_2) LOOP "
                        + "raise_application_error (-20000, 'Данный ресурс уже добавлен для передачи коммунальных ресурсов'); "
                    + "END LOOP; "
                    + "FOR i IN (SELECT res.uuid FROM tb_oki_resources res WHERE res.uuid_oki = :NEW.uuid_oki AND res.code_vc_nsi_2 = :NEW.code_vc_nsi_2) LOOP "
                        + "raise_application_error (-20000, 'Данный ресурс уже добавлен для мощностей'); "
                    + "END LOOP; "
                + "END;"
        );
