    ...
    public OverhaulRegionalProgramHouse () {
        ...
        trigger ("BEFORE INSERT", ""
                + "DECLARE "
                    ...
                    + "cnt NUMBER; "
                + "BEGIN "
                    + "SELECT COUNT(*) INTO cnt FROM tb_oh_reg_pr_houses houses WHERE houses.program_uuid = :NEW.program_uuid AND houses.house = :NEW.house AND houses.is_deleted = 0; "
                    + "IF cnt > 0 THEN "
        ...
        trigger ("BEFORE UPDATE", ""
                ...
                + "BEGIN "
                    + "IF (:OLD.is_deleted = 0 AND :NEW.is_deleted = 1) THEN "
                        + "SELECT COUNT (*) INTO cnt FROM tb_oh_reg_pr_house_work works WHERE works.house_uuid = :OLD.uuid AND works.id_orphw_status = " + VocGisStatus.i.APPROVED.getId () + "; "
                        + "IF cnt > 0 THEN "
