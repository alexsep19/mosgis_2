define ([], function () {
    ...
    $_DO.download_living_room_invalid = function (e) {    
    ...
    $_DO.cancel_living_room_invalid = function (e) {
    ...
    $_DO.edit_living_room_invalid = function (e) {
    ...
    $_DO.update_living_room_invalid = function (e) {
        ...
        if (v.file) {
            ...
            if (!r.f_20132) die ('f_20132', 'Укажите, пожалуйста, основание признания данного помещения непригодным для проживания')
            
            if (!r.f_20133) {
                $('input[name=f_20133]').focus ()
                return alert ('Укажите, пожалуйста, дату документа');
            }

            if (!/^\d\d\.\d\d\.\d\d\d\d$/.test (r.f_20133)) {
                $('input[name=f_20133]').focus ()
                return alert ('Некорректная дата документа');
            }

            if (!r.f_20134) {
                $('input[name=f_20134]').focus ()
            ...
            var check = setInterval (function () {
                ...
                clearInterval (check)
                $_DO.apologize = $.noop
                query ({type: 'living_rooms', action: 'update'}, {data: {f_20132: v.f_20132}}, reload_page)
//                reload_page ()
            }, 100)
