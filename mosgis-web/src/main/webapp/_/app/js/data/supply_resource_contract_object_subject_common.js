define ([], function () {
    ...
    $_DO.cancel_supply_resource_contract_object_subject_common = function (e) {
    ...
    $_DO.edit_supply_resource_contract_object_subject_common = function (e) {
...
//        if (data.item.id_status == 10) die ('foo', 'В настоящий момент данная запись передаётся в ГИС ЖКХ. Операция отменена.')
    ...
    $_DO.update_supply_resource_contract_object_subject_common = function (e) {
    ...
    $_DO.delete_supply_resource_contract_object_subject_common = function (e) {
        if (!confirm ('Удалить эту запись, Вы уверены?')) return
        query ({type: 'supply_resource_contract_subjects', action: 'delete'}, {}, reload_page)
    }

    $_DO.choose_tab_supply_resource_contract_object_subject_common = function (e) {
        ...
        localStorage.setItem ('supply_resource_contract_object_subject_common.active_tab', name)
    ...
    return function (done) {
        ...
        data.active_tab = localStorage.getItem ('supply_resource_contract_object_subject_common.active_tab') || 'supply_resource_contract_object_subject_common_log'
