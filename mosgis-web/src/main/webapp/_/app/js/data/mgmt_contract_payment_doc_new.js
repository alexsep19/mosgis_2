define ([], function () {
    ...
    $_DO.update_mgmt_contract_payment_common_service_payments_popup = function (e) {
        ...
        var f = w2ui ['mgmt_contract_payment_common_service_payments_popup_form']
        ...
        var opt = {
            ...
            onloadend  : function (id) {
                n --
                w2popup.close ()
                var form = w2ui ['mgmt_contract_payment_common_form']
                var fn = 'uuid_voting_protocol'
                var iid = '-' + id
                form.get (fn).options.items.push ({id: iid, text: r.files [0].name})
                form.record [fn] = iid
                form.refresh ()
            }
        }
