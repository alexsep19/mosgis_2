define ([], function () {
    ...
    $_DO.update_overhaul_address_program_docs_new = function (e) {

        var form = w2ui ['overhaul_address_program_docs_new_form']
        ...
        var tia = {type: 'overhaul_address_program_documents'}
        tia.id = form.record.id
        tia.action = tia.id ? 'update' : 'create'
        
        var done = reload_page

        var grid = w2ui ['overhaul_address_program_docs_grid']
        ...
        query (tia, {data: v}, function (data) {
            ...
            if (data.id) w2confirm ('Перейти на страницу документа?').yes (function () {openTab ('/overhaul_address_program_doc/' + data.id)})
