...
public class MeteringDevices extends EJBResource <MeteringDeviceLocal> {
/*    
    ...
    private void checkOrg (JsonObject item) {
        ...
        if (itemOrg == null) throw new InternalServerErrorException ("Wrong MeteringDevice, no org: " + item);
...
//        checkOrg (item);
    ...
    @POST
    @Path("{id}/unset_accounts")
    @Consumes (APPLICATION_JSON)
    @Produces (APPLICATION_JSON)
    public JsonObject doUnsetAccounts (@PathParam ("id") String id, JsonObject p) {
//        final JsonObject item = getInnerItem (id);
//        checkOrg (item);
